...
public class JwtService {
    ...
    private Claims extractAllClaims(String token) {
        try {
            ...
            throw new RuntimeException("Token JWT expiré", e);
        } catch (UnsupportedJwtException e) {
            log.error("Token JWT non supporté: {}", e.getMessage());
            throw new RuntimeException("Token JWT non supporté", e);
        } catch (IllegalArgumentException e) {
            log.error("JWT claims string est vide: {}", e.getMessage());
            throw new RuntimeException("JWT claims string est vide", e);
        }
    }
